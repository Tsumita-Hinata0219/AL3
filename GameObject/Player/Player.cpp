...
void Player::Attack() {
	...
	// 自機から照準オブジェクトのベクトル
	velocity = Subtract(reticleGetWorldPosition(), playerGetWorldPosition());
	velocity = { 
	...
	newBullet->Initialize(model_, playerGetWorldPosition(), velocity);
...
Vector3 Player::playerGetWorldPosition() {
...
Vector3 Player::reticleGetWorldPosition() {
